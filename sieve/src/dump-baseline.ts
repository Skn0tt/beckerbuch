/**
 * Naive data-only SQL dump of finished sieve runs (for fixtures + backups).
 *
 * Dumps **every** finished run with results — corpus and diff-aware alike —
 * so a restore does not silently drop PR / product-change history.
 */
...
async function appendRunDump(
  ...
  const jobs = await pool.query<{
    ...
    `SELECT id, run_id, command, status, attempt, finished_at,
  ...
  const kind = r.baseline_run_id == null ? "corpus" : "diff";
  lines.push(`-- ${kind} run ${r.id} (${r.label}, status=${r.status})`);
  lines.push(
  ...
  if (jobs.rowCount) {
    lines.push(
      `INSERT INTO jobs (id, run_id, command, status, attempt, finished_at, shard_index, test_ids, priority) VALUES`,
    );
    lines.push(
      jobs.rows
        .map(
          (j, i) =>
            `  (${sqlStr(j.id)}, ${sqlStr(j.run_id)}, ${sqlStr(j.command)}, ${sqlStr(j.status)}, ${j.attempt}, ${sqlNullableTs(j.finished_at)}, ${sqlNullableInt(j.shard_index)}, ${sqlTextArray(j.test_ids)}, ${j.priority})${i === jobs.rows.length - 1 ? ";" : ","}`,
        )
        .join("\n"),
    );
    lines.push(``);
...
/**
 * Dump every finished run with results (corpus + diff-aware).
 * Corpus/mainline rows are written first so `baseline_run_id` FKs resolve.
 */
  ...
  try {
    const runs = await pool.query<{ id: string }>(
      `SELECT r.id
       ...
       ORDER BY
         CASE WHEN r.baseline_run_id IS NULL THEN 0 ELSE 1 END,
         r.finished_at ASC NULLS LAST,
         r.created_at ASC`,
    );
    if (!runs.rowCount) {
      throw new Error("no finished runs with results to dump");
    }

    const lines: string[] = [
      `-- Data-only seed: ALL finished runs with results (corpus + diff-aware).`,
      `-- Generated by dumpDatabaseSql (${runs.rowCount} run(s)).`,
      `-- Apply after schema.sql migrate. Naive full history — do not filter.`,
      ``,
      `BEGIN;`,
      ``,
    ];
...
/** @deprecated Use {@link dumpDatabaseSql} — kept as an alias. */
export const dumpCorpusSql = dumpDatabaseSql;
...
export async function writeDatabaseBackups(
  databaseUrl: string,
  ...
  const stamp = new Date()
    ...
    .replace(/[-:]/g, "")
    .replace(/\.\d{3}Z$/, "Z");
  const stampedPath = path.join(backupDir, `dump-${stamp}.sql`);
  const latestPath = path.join(backupDir, "dump-latest.sql");
  // Keep the old corpus-* names as copies so existing restore habits still work.
  const stampedCorpusPath = path.join(backupDir, `corpus-${stamp}.sql`);
  const latestCorpusPath = path.join(backupDir, "corpus-latest.sql");

  const { runCount, resultCount } = await dumpDatabaseSql(
    databaseUrl,
    fixturePath,
  );
  await copyFile(fixturePath, stampedPath);
  await copyFile(fixturePath, latestPath);
  await copyFile(fixturePath, stampedCorpusPath);
  await copyFile(fixturePath, latestCorpusPath);

  return { fixturePath, stampedPath, latestPath, runCount, resultCount };
}

/** @deprecated Use {@link writeDatabaseBackups} — kept as an alias. */
export const writeCorpusBackups = writeDatabaseBackups;
